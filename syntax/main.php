...
class syntax_plugin_diagram_main extends DokuWiki_Syntax_Plugin
{
	...
	function _genCommandsAndAbbrs ($calls)
	{
		...
		foreach ($calls as $call)
		{
			...
			{
				$abbr_met = true;
				$abbrs[$line_index][$data['abbr']] = array();
				$catching_abbr = &$abbrs[$line_index][$data['abbr']];
				continue;
			}
	...
	function _genFramework ($commands)
	{
		...
		// solid border for box cell
		$border_bs = '2px solid black';
		...
		for ($i = 0, $ir = 0; $i < $res['line_count']; $i++, $ir += 2)
		{
			for ($j = 0, $jr = 0; $j < count($commands[$i]); $j++)
			{
				...
				switch ($cell_text)
				{
					...
					default:
						$res[$ir][$jr] = $this->_boxCell(6, 2, $cell_text, $border_bs);
	 ...
	 */
	function _boxCell ($width, $height, $abbr, $border, $background_color = null)
	{
		$style = "text-align: center; padding: 0.25em; border: $border;";
			...
			foreach ($row as $cell)
			{
				...
				if (is_null($cell['abbr']))
					$cell_content = '<div style="width: '
						...
						.$cell["rowspan"]
						.'em;"><span>&nbsp;</span></div>';
				else if (array_key_exists($i/2, $abbrs) && array_key_exists($cell['abbr'], $abbrs[$i/2]))
					$cell_content = $this->_renderWikiCalls ($abbrs[$i/2][$cell['abbr']]);
				else
